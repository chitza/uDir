    ...
    public partial class MainForm : Form
    {
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            else if (m.Msg == WinAPI.WM_COPYDATA)
            {
                ...
                ShowWindow();
                var dataStruct = (WinAPI.CopyDataStruct)m.GetLParam(typeof(WinAPI.CopyDataStruct));
                string data = dataStruct.Data;
                logger.Log(LogLevel.Trace, "\tdata: " + data);

                m.Result = new IntPtr(1); //return "True" to caller

                ShowNotification("Received message: " + data);
                if (!model.LoadTorrent(data))
                    ShowNotification("Error loading torrent " + data);
            }
            else
                base.WndProc(ref m);
        }
